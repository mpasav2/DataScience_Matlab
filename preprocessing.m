function output_data = preprocessing(param, vari, S0x, opt)
output_data = {};
           ...
           for ff=1:length(files)
               ...
               elseif case_num ==2
                   if contains(vari, 'LHipMoment') && strcmp(S0x(trial+ff).Exo, 'exo')
                       ...
                       L_ASIS= S0x(trial+ff).L_ASIS(1:3, :);
                       L_ASIS(:, 2) = L_ASIS(:, 2) - 0.03;
                       % Calculate the vector difference LB1 - LB2
                       vector_diff = LB1 - LB2;
                       vector_diff1=L_ASIS-LB2;
                       % Compute the magnitude (norm) of the vector
                       magnitude= sqrt(sum(vector_diff.^2, 1));
                       % Calculate the displacement vector from RB1 to RB2
                       torque = cross(vector_diff1, ((LB1 - LB2)/magnitude) .* loadcell);
                       HipMoment = [1;1;1].*(S0x(trial+ff).LHipMoment);
                       % Swap the first and second columns
                       torque_1 = torque(2,:);
                       torque_1 = [torque_1; torque(1,:)];
                       torque_1 = [torque_1; torque(3,:)];
                       torque_truncated = torque_1;
                       torque_truncated = torque_truncated(1:size(HipMoment, 1), :);
                       %torque_truncated(:, [1, 2]) = torque_truncated(:, [2, 1]);
                       %result_column = torque_truncated(:, 2) + HipMoment(:, 1);
                   ...
                   elseif contains(vari,'RHipMoment') && strcmp(S0x(trial+ff).Exo, 'exo')
                       ...
                       R_ASIS= S0x(trial+ff).R_ASIS(1:3, :);
                       R_ASIS(:, 2) = R_ASIS(:, 2) + 0.03;
                       vector_diff = RB1 - RB2;
                       vector_diff1=RB2-R_ASIS;
                       % Compute the magnitude (norm) of the vector
                       magnitude = norm(vector_diff);
                       % Calculate the displacement vector from RB1 to RB2
                       torque = cross(vector_diff1 , ((RB1 - RB2)/magnitude) .* loadcell);
                   ...
                   elseif (contains(vari, 'LHipMoment') || contains(vari, 'RHipMoment')) && strcmp(S0x(trial+ff).Exo, 'noexo')
                       indx = vari(end-3);
                       if str2num(indx) == 1
               ...
               %if files(ff) ~= 16 % Keep it only for S04
