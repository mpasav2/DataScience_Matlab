...
timelength = 350;% S01=350,S02=350, S03=400, S04=350,S05=380, C01=400
for ss = 2 % Choose subject to plot, from sublist = {'S01','S02','S03','S04','C05','C06'};
    figure;
    ...
    for cc = 1:4
        ...
        for tt = 1:size(Data(cc).HABD, 1)
            ...
            if plotornot == 1
                ...
                plot(Data(cc).HEF(tt,:))
                hold on
                [~, p_indx_min] = min(Data(cc).HEF(tt,1:timelength));
                scatter(p_indx_min, min(Data(cc).HEF(tt,1:timelength)), 'red', 'filled')
                hold off
                drawnow
                ...
                hold on
                %[~, p_indx_max] = max(Data(cc).HABD(tt,1:timelength));
                scatter(p_indx_min, Data(cc).HABD(tt, p_indx_min), 'green', 'filled')
                hold off
                drawnow
            ...
            % Select the abduction torque at the index of the minimum hip extension torque
            accumHABDpeak = [accumHABDpeak, Data(cc).HABD(tt, p_indx_min)];
            HABDpeak_cond = [HABDpeak_cond, Data(cc).HABD(tt, p_indx_min)];
            
            accumHEFpeak = [accumHEFpeak, min(Data(cc).HEF(tt,1:timelength))];
            HEFpeak_cond = [HEFpeak_cond, min(Data(cc).HEF(tt,1:timelength))];
    
            subplot(2,2,2)
            scatter(abs(min(Data(cc).HEF(tt,1:timelength))), abs(Data(cc).HABD(tt, p_indx_min)), 250, Colors(cc,:), '.')   % peaks from individual trial
            hold on
        end
        ...
        hold on
        errorbar(cc, mean(abs(HABDpeak_cond)), std(abs(HABDpeak_cond)), 'k', 'LineWidth', 2, 'CapSize', 10)
        if cc == 4
            box off
            ubhabd = max(accumHABDpeak) * 1.1;
            ylim([0 ubhabd])
            ylabel('HABD Angle ( °)')
        ...
        subplot(2,2,4)
        barh(fliporder(cc), mean(abs(HEFpeak_cond)), 'EdgeColor', 'none', 'FaceColor', Colors(cc,:))
        hold on
        errorbar(mean(abs(HEFpeak_cond)), fliporder(cc), std(abs(HEFpeak_cond)), "horizontal", 'k', 'LineWidth', 2, 'CapSize', 10)
        if cc == 4
            box off
